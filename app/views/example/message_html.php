<article>
	...
	&#09;return View::render_layout('default', $this);
		...
		<li>
			<p>The static View::render method will look in "views/example/" for a file called "index_html.php". The framework determines the file type (eg. html, xml, json, rss, atom, etc.) based on the url. If the url is "/example.html", then the framework will set the resource's fileType property to "html" and View::render will return the views/example/index_html.php file to the resource's "output" property.</p>
		</li>
		<li>
			<p>The return call in the function above render's the output property in the default layout which resides in "views/layouts/default_html.php", where the "_html" in the file name indicates it's representation (The Re in REST).</p>
		</li>
		<li>
			<p>You'll also notice a themes folder in the file hierarchy. This folder contains themes that you might want to use for your site. There's a "default" theme in the code base now which can be configured by returning the name of the theme folder in the Settings::$theme property. If the "views" folder hierarchy is defined in a theme folder within the "themes" folder, the framework will look there first to find any views it's looking for. The "views" functionality is how the framework implements the "Representational" part of REST. You can represent the data in any way by creating a view for it, according to the file type you specify. For instance, if I want to represent ".doc" files, I would create view files appended with "_doc.php". Check out the example resource and views to see what I mean.</p>
		</li>
	</ol>
</article>